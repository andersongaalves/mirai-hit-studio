...
class OpenAIProvider:
    def __init__(self, api_key, model, *, timeout=8, transport=None):
    ...
    def generate(self, incoming):
        ...
        try:
            ...
            for item in data["output"]:
                if item["type"] == "function_call":
                    ...
                    if item["name"] == "request_handoff":
                        return ProviderResponse(handoff_reason=HandoffReason(arguments["reason"]))
            ...
            return ProviderResponse(tool_calls=tuple(calls)) if calls else ProviderResponse(text="\n".join(texts))
