...
async function staticResponse(route) {
    ...
    const pathname = url.pathname === "/creators" ? "/creators.html" : url.pathname;
...
(async () => {
    ...
    try {
        ...
        for (const width of [320, 375, 390, 414, 768, 1024, 1440]) {
            ...
            assert.ok(overflow <= 1, `creators overflow at ${width}px: ${overflow}px`);
        }
        assert.deepEqual(errors, []);
        console.log("PASS: Creators offers, applications, consented analytics and responsive structure.");
    } finally {
        await browser.close();
    }
})().catch(error => {
